...
export async function generateBrickConfig(
  ...
  for (const layout of layouts) {
    ...
    for (const zone of zones) {
      ...
      // Image is now queried by zoneId (FK relationship)
      const zoneImage = await storage.getZoneImageByZoneId(zone.id);
      const imageId = zoneImage?.id || '';
  ...
  for (const user of usersResult.data) {
    ...
    // CRITICAL: Must filter by companyId to prevent cross-company data leakage
    const fullUser = await storage.getUserById(user.userId, companyId);
    if (fullUser) {
